...
export default function AdminPage() {
  ...
  async function handleRestock(claim: any) {
    ...
    await supabase
      .from("products")
      .update({ stock: (claim.products.buy_price !== undefined ? undefined : undefined) })
      .eq("id", claim.product_id);

    // Voorraad ophogen met 1
    ...
    await supabase
      .from("claims")
      .update({ status: "verlopen_verwerkt" })
      .eq("id", claim.id);
  ...
  return (
    <main className="min-h-screen bg-gray-50 px-6 py-12">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-2xl font-bold mb-8">Adminpaneel</h1>

        <form
          onSubmit={handleAddProduct}
          className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-10 space-y-4"
        >
          ...
          <input
            placeholder="Foto-URL (bijv. https://...)"
            value={imageUrl}
        ...
        <h2 className="font-semibold mb-4">
          Bestaande producten ({products.length})
        </h2>
        <div className="space-y-3 mb-12">
          {products.map((product) => (
            <div
              key={product.id}
              className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex justify-between items-center gap-4"
            >
              {product.image_url && (
                <img
        ...
        <h2 className="font-semibold mb-4">
          Openstaande verkopen ({claims.length})
        </h2>
        {claims.length === 0 ? (
          <p className="text-gray-400 text-sm mb-12">
            Er zijn nog geen gemelde verkopen.
          </p>
        ) : (
          <div className="space-y-3 mb-12">
            {claims.map((claim) => (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Verkoper: {claim.profiles?.email} · Inkoopprijs te ontvangen: €
                    {claim.products?.buy_price} · Winst uit te betalen: €{claim.profit}
                  </p>
                </div>
                {claim.status === "verkocht" ? (
                  <button
                    onClick={() => handleMarkPaid(claim.id)}
                    className="bg-black text-white rounded-lg px-4 py-2 text-sm font-semibold hover:bg-gray-800 transition flex-shrink-0"
                  >
                    Markeer als afgerond
                  </button>
                ) : (
                  <span className="text-xs font-medium px-3 py-1 rounded-full bg-green-100 text-green-800 flex-shrink-0">
                    Afgerond
                  </span>
                )}
              </div>
        ...
        <h2 className="font-semibold mb-4">
          Verlopen claims ({expiredClaims.length})
        </h2>
        {expiredClaims.length === 0 ? (
          <p className="text-gray-400 text-sm">
            Er zijn geen verlopen claims die nog verwerkt moeten worden.
          </p>
        ) : (
          <div className="space-y-3">
            {expiredClaims.map((claim) => (
              <div
                key={claim.id}
                className="bg-white rounded-xl p-4 shadow-sm border border-red-100 flex justify-between items-center gap-4"
              >
                <div>
                  <p className="font-medium">{claim.products?.name}</p>
                  <p className="text-sm text-gray-500">
                    Geclaimd door: {claim.profiles?.email} — niet op tijd verkocht
                  </p>
                </div>
                <button
                  onClick={() => handleRestock(claim)}
                  className="bg-red-600 text-white rounded-lg px-4 py-2 text-sm font-semibold hover:bg-red-700 transition flex-shrink-0"
                >
